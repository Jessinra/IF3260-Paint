...
void MoveableObject::move() {
    this->position.setPoint(this->position.getX() + this->speed * this->dx,
                            this->position.getY() + this->speed * this->dy);
}
...
void MoveableObject::selfStretchX(float pivotX, float pivotY, float scalingConstant) {
    pivotX -= position.getX();
    pivotY -= position.getY();

    for (MoveablePlane &plane : this->planes) {
        plane.selfStretchX(pivotX, pivotY, scalingConstant);
    }

    calculate();
}

void MoveableObject::selfStretchY(float pivotX, float pivotY, float scalingConstant) {
    pivotX -= position.getX();
    pivotY -= position.getY();

    for (MoveablePlane &plane : this->planes) {
        plane.selfStretchY(pivotX, pivotY, scalingConstant);
    }
